...
use std::thread;
use symcts::{
    coverage::SyMCTSCoverageFeedback,
    metadata::{global::SyMCTSGlobalMetadata, solve_stats::SymbolicSolveStats},
    reproducibility_details::{dump_build_details, dump_binary_and_deps},
    util::{hash_bytes, ensure_baseline_inputs_exist},
};
use std::{
    path::{PathBuf, Path},
...
#[cfg(not(feature="scheduling_symcc"))]
use symcts::symcts_scheduler::SyMCTSScheduler as Scheduler;
#[cfg(feature="scheduling_symcc")]
use symcts::symcc_scheduler::SymCCScheduler as Scheduler;
...
use libafl::{
    ...
    Error, prelude::{TimeoutForkserverExecutor, StdMapObserver, ForkserverExecutor, Monitor, EventFirer, ProgressReporter},
};

#[cfg(feature = "sync_from_other_fuzzers")]
use symcts::sync_from_afl_stage::SyncFromAFLStage;
...
pub struct SyMCTSArgs {
    ...
    #[clap(long, help="Concolic execution mode to use. Either `symcc` or `symqemu`.")]
...
pub fn main() -> Result<(), Error> {
    ...
    let (mut feedback, mut cov_executor) = {
        ...
        let symcts_feedback = SyMCTSCoverageFeedback::for_afl_bitmap_observer(&afl_map_observer, time_coverage.name());
    ...
    let symcts_scheduler = Scheduler::new();
    ...
    #[cfg(not(feature="sync_from_other_fuzzers"))]
    let mut stages = tuple_list!(
        // Create a concolic trace
        ConcolicTracingStage::new(TracingStage::new(concolic_executor), concolic_observer_name),
        // Use the concolic trace for z3-based solving
    ...
    manager.maybe_report_progress(&mut state, Duration::from_secs(0), Duration::from_secs(1))?;
...
fn dump_run_info(my_sync_dir: &PathBuf, args: &SyMCTSArgs) {
    ...
    // lastly, dump this binary itself
    let current_bin = std::env::current_exe().unwrap();
    dump_binary_and_deps(&bin_dir.join("symcts"), &current_bin);
}
