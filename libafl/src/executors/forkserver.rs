...
impl Forkserver {
    ...
    pub fn with_kill_signal(
        ...
        let fsrv_handle = match ConfigTarget::setsid(command
            .env("LD_BIND_NOW", "1")
            .envs(envs)
            .setlimit(memlimit)
        )
    ...
    SP: ShMemProvider,
{
    ...
    #[inline]
    pub fn execute_input_uncounted(
        &mut self,
        input: &[u8],
        ...
        let mut input_bytes: OwnedSlice<'_, u8> = input.to_owned().into();
    ...
    Z: UsesState<State = S>,
{
    ...
    fn run_target(
        ...
        *state.executions_mut() += 1;
        self.execute_input_uncounted(input.target_bytes().as_ref())
    }
}
