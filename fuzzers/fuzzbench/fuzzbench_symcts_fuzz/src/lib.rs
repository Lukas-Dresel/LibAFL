...
use libafl_targets::edges_map_mut_slice;
use libafl::prelude::{StdMapObserver, TracingStage, StdScheduledMutator};
use libafl::{
    Error, HasMetadata,
    ...
    feedbacks::{CrashFeedback, MaxMapFeedback, AlwaysInterestingMapFeedback, TimeFeedback},
    fuzzer::{Fuzzer, StdFuzzer},
    inputs::{BytesInput, HasTargetBytes},
    monitors::SimpleMonitor,
    mutators::{
        StdMOptMutator, Tokens, havoc_mutations,
...
use symcts_fuzz::{util::ensure_baseline_inputs_exist, coverage::SyMCTSCoverageFeedback, metadata::{global::SyMCTSGlobalMetadata, solve_stats::SymbolicSolveStats}, symcts_scheduler::SyMCTSScheduler};
...
pub extern "C" fn libafl_main() {
    ...
    if let Err(e) = fuzz(out_dir, crashes, &in_dir, tokens, &logfile, timeout)
    {
        eprintln!("Fuzzing failed: {e}");
        panic!("Fuzzing failed: {e}");
    }
}
...
fn fuzz(
    ...
    let edges_observer2 = unsafe { std_edges_map_observer("edges") };
    ...
    let mut symcts_feedback = SyMCTSCoverageFeedback::for_afl_bitmap_observer(&edges_observer2, time_observer.name().clone());
    ...
    let mut feedback = feedback_or!(
        // symcts feedback linked to the edges observer
        symcts_feedback,
    ...
    global_meta.sync_dir = corpus_dir.parent().expect("Expected to be able to walk up out of the queue directory.").into();
    state.add_metadata(global_meta);
    state.add_metadata(SymbolicSolveStats::default());
    // ensure_baseline_inputs_exist(&corpus_dir).unwrap();
    ...
    let scheduler = SyMCTSScheduler::new(scheduler);
    ...
    let mut executor = InProcessExecutor::with_timeout(
        &mut harness,
        tuple_list!(edges_observer, edges_observer2, time_observer),
        &mut fuzzer,
        &mut state,
        &mut mgr,
        timeout,
