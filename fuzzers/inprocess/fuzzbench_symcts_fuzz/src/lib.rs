...
use symcts::{util::ensure_baseline_inputs_exist, coverage::SyMCTSCoverageFeedback, metadata::{global::SyMCTSGlobalMetadata, solve_stats::SymbolicSolveStats}, symcts_scheduler::SyMCTSScheduler};
...
fn fuzz(
    ...
    let edges_observer = unsafe {
            let mut slice_of_u8s = edges_map_mut_slice();
            let mut slice_of_u32s = unsafe {
                std::slice::from_raw_parts_mut(
                    slice_of_u8s.as_mut_ptr() as *mut u32,
                    slice_of_u8s.len() / std::mem::size_of::<u32>(),
                )
            };
            StdMapObserver::from_mut_slice("edges", OwnedMutSlice::from(slice_of_u32s))
        };

    let mut symcts_feedback = SyMCTSCoverageFeedback::for_afl_bitmap_observer(&edges_observer);
    ...
    // let calibration = CalibrationStage::new(&symcts_feedback);
    ...
    let mut fuzzer = StdFuzzer::new(symcts_scheduler, symcts_feedback, objective);
