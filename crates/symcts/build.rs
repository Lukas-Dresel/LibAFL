...
fn find_cargo_lock_robust() -> Result<PathBuf, String> {
    let manifest_dir = env::var("CARGO_MANIFEST_DIR")
        .map_err(|_| "CARGO_MANIFEST_DIR not set")?;
    let start_path = PathBuf::from(&manifest_dir);

    // Check manifest directory first
    let local_lock = start_path.join("Cargo.lock");
    if local_lock.exists() {
        return Ok(local_lock);
    }
    ...
    let manifest_dir = env::var("OUT_DIR")
        .map_err(|_| "OUT_DIR not set")?;
    let start_path = PathBuf::from(&manifest_dir);
    ...
    Err(format!("No Cargo.lock found in {}", manifest_dir))
}
...
fn dump_build_details() {
    ...
    let cargo_lock = std::fs::read_to_string(find_cargo_lock_robust().unwrap()).unwrap();
    OpenOptions::new()
        .write(true)
        .create(true)
        .truncate(true)
        .open(out_dir.join("Cargo.lock.rs"))
        .unwrap()
        .write_all(cargo_lock.as_bytes())
        .unwrap();
}
