...
use libafl::prelude::{HasTestcase, HasTargetBytes};
...
fn num_times_mutated<I, C: Corpus<I>>(corpus: &C, id: CorpusId) -> usize {
    corpus
        .get(id)
        .unwrap()
        .borrow()
...
fn trace_len<I, C: Corpus<I>>(corpus: &C, id: CorpusId) -> usize {
    corpus
        .get(id)
        .unwrap()
        .borrow()
...
fn input_len<I, C: Corpus<I>>(corpus: &C, id: CorpusId) -> usize
where 
    I: HasLen
{
    corpus
...
impl<I, S> Scheduler<I, S> for SyMCTSScheduler<S>
where
    S: HasRand + HasMetadata + HasTestcase<I> + BetterStateTrait<I>,
    I: HasLen + HasTargetBytes,
{
    fn next(&mut self, state: &mut S) -> Result<CorpusId, Error> {
        ...
        if let Some((scheduled_coverage_point, scheduled_coverage_info)) = scheduled {
            ...
            register_symbolic_sampling_of_testcase(state, least_covered_id);
            ...
            register_symbolic_sampling_of_testcase(state, *corpus_id);
            return Ok(*corpus_id)
        }
    }
    ...
    fn on_add(&mut self, state: &mut S, inserted_idx: CorpusId) -> Result<(), libafl::Error> {
        let input_hash = {
            ...
            hash_target_bytes_input(testcase.load_input(corpus)?)
        };
