...
use libafl::{Error, prelude::{format_duration_hms, ExitKind}};
...
use std::{
    ...
    path::{PathBuf, Path},
    time::{SystemTime, UNIX_EPOCH}, str::FromStr, ops::{Not, BitAnd, BitAndAssign},
};
...
impl Classification {
    ...
    fn get_instance_run_tuple(&self) -> InstanceRunTuple {
        (self.instance_type.clone(), self.instance_num)
    }
}
...
impl ExperimentTracedInput for TracedInput {
    ...
    fn get_classification_magma(&self, args: &Args) -> Classification {
        ...
        let full_instance = format!("{}_{}", instance_type, instance_num_dir);
...
fn fuzz(args: &Args) -> Result<(), Error> {
    ...
    for (target, result) in result {
        ...
        for (target, accumulated_bitmap) in accumulated_bitmaps_by_classification.iter() {
            ...
            let all_edges = accumulated_bitmap.edge_first_seen_at_hit.iter()
                                .map(|(label, by_edge)| by_edge.keys())
                                .flatten()
                                .unique()
                                .sorted()
                                .collect::<Vec<_>>();
